...
const SingleForm = (props) => {
  return (
    ...
    <div className={`${styles["single-form"]}`}>
        <label for="uname" className= {`${styles[props.textColor]}`}><b>{props.title}</b></label>
        <input className={`${styles["input"]}`} type="text"  onChange = {props.handleChange} name="uname" required></input>
        <StandardButton buttonType="submit" buttonText={props.buttonText} buttonStyle="green-button" />
